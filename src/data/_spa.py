"""Vendored NREL SPA solar-position algorithm (pure NumPy).

Copied verbatim from pvlib 0.15.2 (``pvlib/spa.py``, BSD-3-Clause;
Copyright (c) 2013-2025 pvlib python contributors) so the serverless API
runs the exact same solar geometry without installing pvlib/scipy/h5py.

Algorithm: I. Reda and A. Andreas, "Solar position algorithm for solar
radiation applications", Solar Energy 76(5):577-589, 2004 (+ 2007
corrigendum). NREL SPA is public domain; this translation is from pvlib.

Only the numpy code path is used (no numba). See src/data/solar.py for
the project-facing wrapper and validation script scripts/validate_solar_math.py.
"""
"""
...
# MODIFIED (vs upstream pvlib): the numba branch is disabled on purpose.
# pvlib's default method 'nrel_numpy' reloads the module with numba off and
# runs this same numpy path, so forcing numpy keeps our results bit-identical
# to the reference while removing the numba dependency for the serverless API.
if False:                       # pragma: no cover - numba path disabled
    try:
