...
def build_static() -> None:
    ...
    # climate fallback bundle is generated by scripts/build_climate_bundle.py
    data_src, data_out = SRC / "data", OUT / "data"
    if data_src.is_dir():
        data_out.mkdir(exist_ok=True)
        for f in sorted(data_src.glob("*.json")):
            (data_out / f.name).write_bytes(f.read_bytes())
            print(f"[build] data/{f.name} {f.stat().st_size:,} bytes")
