...
class NLPRequest(BaseModel):
    ...
    #: Opaque id from `dialogue.session` of the first reply. Echoing it back
    #: enables the server-side undo stack and turn counter (roadmap Step 3.4)
    #: across clients that keep no state of their own.
...
#: Engineering envelope the assistant's answers are clamped to (roadmap
#: Step 3.5). These are the documented limits for parameters the RC core can
#: integrate meaningfully, quoted from the SIH dataset doc Part 3.5; the
#: numbers reported back are always post-clamp, so no reply can advertise a
#: geometry the solver would not accept. A clamp is REPORTED in `applied`,
#: never silent.
NLP_ENGINE_LIMITS = {
    "length_m": (2.0, 15.0), "width_m": (2.0, 15.0), "height_m": (2.0, 4.5),
    "wall_thickness_m": (0.05, 0.60), "roof_thickness_m": (0.001, 0.50),
    # 0.001 m floor so real thin metal-sheet roofs (the 2 mm GI presets) sit
    # inside the range instead of being clamped upward
    "insulation_thickness_m": (0.0, 0.25), "ach": (0.5, 12.0),
    "roof_pitch_deg": (0.0, 35.0), "window_width_m": (0.3, 5.0),
    "window_height_m": (0.3, 5.0), "window_shgc": (0.1, 0.95),
    "window_u_w_m2k": (0.5, 12.0),
}


def _nlp_clamp(design: dict) -> tuple[dict, list[str]]:
    """Clamp every bounded key into NLP_ENGINE_LIMITS; report each change."""
    notes = []
    for key, (lo, hi) in NLP_ENGINE_LIMITS.items():
        v = design.get(key)
        if v is None:
            continue
        try:
            ...
            continue
        if not lo <= f <= hi:
            design[key] = round(min(max(f, lo), hi), 4)
            notes.append(f"{key} {f:g} -> {design[key]:g} "
                         "(outside the engine-safe range)")
    return design, notes
...
def nlp_info():
    ...
    # Step 3.7 benchmark table, regenerated by ml/nlp/eval_nlp.py — served
    # as-measured, with its method notes, so quoted numbers are auditable
    benchmark = None
    try:
        benchmark = json.loads(
            (nlp_design.MODEL_FILE.parent / "nlp_benchmark.json")
    ...
    return {
        ...
        "benchmark": benchmark,
        "capabilities": ["grammar-rules", "hinglish-bridge", "compare-engine",
                         "fuzzy-gazetteer", "imperial-units", "clamp-guard",
                         "undo", "turn-counter", "feedback-loop"],
...
def nlp_design_endpoint(req: NLPRequest):
    ...
    # normalisation honesty (roadmap 3.3): whenever the parser GUESSED a
    # spelling or CONVERTED a unit, say so — a silent "thermocoal→EPS" would
    # be exactly the kind of unexplained inference this assistant avoids.
    hints = [f"read '{k}' as {v}" for k, v in (slots.get("fuzzy_read") or {}).items()]
    hints += [f"{v} (imperial units converted)"
              for v in (slots.get("unit_conversions") or {}).values()]
    if hints:
        understood["hints"] = hints

    # conversation state (roadmap 3.4): the client's context_design remains
    # the source of truth; the session adds the undo stack + turn counter on
    # top, seeded from it so a mid-conversation tab reload still lands right.
    sid = dst_manager.open_session(req.session_id, req.context_design)
    if slots.get("undo"):
        # undo means "give me back the design before that last change" — the
        # engine re-runs on the RESTORED state; numbers are never replayed
        restored = dst_manager.pop_undo(sid)
        if restored is None:
            return {"understood": understood, "actionable": False,
                    "kind": "undo", "site": site_name or "current pin",
                    "dialogue": dst_manager.state(sid),
                    "message": ("Nothing to undo yet in this conversation — "
                                "make a change first, then say \"undo\".")}
        design = dict(_flat_design(CFG))
        design.update({k: v for k, v in restored["design"].items()
                       if k in design and v is not None})
        applied_u = [f"undid: \"{restored['undid_text']}\" — previous design restored"]
        ignored_u: list[str] = []
        # instructions riding along with the undo still apply on top
        mats_u = load_materials()
        for _k in ("wall_material", "roof_material", "insulation_material",
                   "wall_thickness_m", "roof_thickness_m",
                   "insulation_thickness_m", "length_m", "width_m",
                   "height_m", "window_wall", "window_width_m",
                   "window_height_m", "ach", "orientation_deg",
                   "roof_pitch_deg", "window_u_w_m2k", "window_shgc"):
            if _k not in slots:
                continue
            if _k.endswith("_material") and _k != "insulation_material" \
                    and slots[_k] not in mats_u.index:
                ignored_u.append(f"{_k}={slots[_k]} (not in the materials table)")
                continue
            design[_k] = slots[_k]
            applied_u.append(f"{_k} = {slots[_k]}")
        design, clamp_notes = _nlp_clamp(design)
        applied_u += clamp_notes
        try:
            _w, _src, _ = get_weather_cached(lat, lon, _period_year(period),
                                             tz, period=period)
            weeks = design_weeks(_w, _period_year(period))
            _cfg = _apply_ground_temp(_apply_site_location(
                _apply_design(CFG, design), lat, lon, tz), _w)
            _res = simulate(_cfg, weeks["hot_week"], mats_u)
            _st = comfort_stats(_res, CFG["climate"]["comfort_range_c"])
            metrics = {k: (round(float(v), 3) if isinstance(v, float) else v)
                       for k, v in _st.items()}
        except Exception as exc:                            # noqa: BLE001
            metrics, _src = None, None
            print(f"[api] nlp undo simulate failed: {exc}")
        return {
            "understood": understood, "actionable": True,
            "intent": "modify", "kind": "undo",
            "site": site_name or "current pin",
            "design": design,
            "applied": applied_u,
            "ignored": ignored_u,
            ...
            "metrics": metrics,
            "dialogue": dst_manager.state(sid),
            "message": ("Restored the design from before that change; "
    ...
    for key, factor in (rel.items() if req.context_design else []):
        if key == "size":
            lo, hi = NLP_ENGINE_LIMITS["length_m"]
            for dim in ("length_m", "width_m"):
                design[dim] = round(min(max(float(design[dim]) * factor, lo), hi), 2)
            applied.append(f"size x{factor} → {design['length_m']} x {design['width_m']} m")
            continue
        cur = design.get(key)
        if cur is None:
            continue
        # same limits table as the final clamp — one source of engineering
        # truth for the assistant (roadmap 3.5); before this the relative
        # path had a private copy with a 0.3 ACH floor and a 0.3 m insulation
        # cap that disagreed with it
        lo, hi = NLP_ENGINE_LIMITS.get(key, (0.0, 1e9))
    ...
    # engineering clamps (roadmap 3.5) run AFTER every edit so the reported
    # design and metrics are the ones the engine actually integrated — and
    # every clamp is announced rather than swallowed
    design, clamp_notes = _nlp_clamp(design)
    applied += clamp_notes

    turn_no = dst_manager.record_turn(sid, text, design, applied)
    ...
    out = {
        ...
        "ignored": ignored,
        "weather_source": source,
        "dialogue": {"session": sid, "turn": turn_no,
                     "undo_depth": dst_manager.state(sid)["undo_depth"]},
        "message": _nlp_message(intent, confidence, site_name, slots),
    }
...
#: roadmap Step 3.6: retrain once this many DISLIKED parses carrying a
#: user correction have accumulated (the dataset doc's "100 new entries"
#: trigger). The trigger is reported, never auto-applied silently.
NLP_RETRAIN_THRESHOLD = 100


@app.get("/api/nlp/feedback/stats")
def nlp_feedback_stats():
    """Active-learning backlog: how much real user signal is waiting.

    Feeds scripts/retrain_nlp.py — the honest, inspectable half of the
    "autonomous retraining pipeline": production accumulates rows and
    reports the count; the actual weight update is a human-run script that
    re-verifies the hold-outs before any model file is replaced.
    """
    try:
        rows = STORE.list_nlp_feedback(limit=5000)
    except Exception as exc:                            # noqa: BLE001
        return {"ok": False, "error": str(exc), "total": 0, "wrong": 0,
                "with_correction": 0, "pending_retrain": 0,
                "threshold": NLP_RETRAIN_THRESHOLD}
    wrong = [r for r in rows if not r.get("correct")]
    pending = [r for r in wrong if (r.get("correction") or "").strip()]
    return {
        "ok": True,
        "total": len(rows),
        "wrong": len(wrong),
        "with_correction": len(pending),
        "pending_retrain": len(pending),
        "threshold": NLP_RETRAIN_THRESHOLD,
        ...
        "note": ("Rows land here from the SEC/11 verdict buttons. "
                 ...
                 "folds them into the next training run behind the same "
                 "hold-out gate as every other model change."),
    }
