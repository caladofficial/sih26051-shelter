...
jobs:
  refresh:
    ...
    steps:
      ...
      # then fetches only the new tail from Open-Meteo — ~90 s instead of the
      # ~13 min a full re-scrape took, and no longer five-sites-time-out
      # flaky. Use --full to force a complete re-download.
